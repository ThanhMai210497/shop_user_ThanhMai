...
/*  GLOBAL STATE */
let products = []; // mảng Product load từ "api"
...
/* RENDER NAV + HERO handled in HTML; here render product grid, modal, cart */

/* 1) Render product grid */
export async function initShop() {
  try {
    const res = await api.fetchProducts();
    products = res.data.map((p) => new Product(p));
  ...
  $("#sortSelect").addEventListener("change", (e) => {
    ...
    else filtered = filtered; // default keep
    renderProducts(filtered);
  });
}
...
function renderCategories() {
  ...
  $("#categorySelect").addEventListener("change", (e) => {
    ...
    if (val === "Tất cả") filtered = products.slice();
    ...
    <article class="card" data-id="${p.id}">
      ...
      <div class="card-body">
        ...
        <div class="card-actions">
          <button class="btn btn-outline btn-detail" data-id="${
            p.id
          }">Xem</button>
          <button class="btn btn-primary btn-add" data-id="${
            p.id
          }">Thêm vào giỏ</button>
        </div>
      </div>
...
/* Init: mount to window so HTML can call initShop() from script tag */
window.shop = {
  initShop,
  toggleCartSidebar,
};
